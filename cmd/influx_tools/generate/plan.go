...
func (p *Plan) Info() *meta.DatabaseInfo {
	return p.info
}
...
func (p *Plan) InitMetadata(client server.MetaClient) (err error) {
	...
	p.info, err = client.CreateDatabaseWithRetentionPolicy(p.Database, &rp)
	if err != nil {
		return err
	}

	return p.createShardGroupMetadata(client)
}
...
func (p *Plan) InitFileSystem(client server.MetaClient) error {
	...
	groups, err := client.ShardGroupsByTimeRange(p.Database, p.Retention, minT, maxT)
	if err != nil {
		return err
	}

	for i := 0; i < len(groups); i++ {
		if err = os.MkdirAll(filepath.Join(p.ShardPath(), strconv.Itoa(int(groups[i].ID))), 0777); err != nil {
			return err
		}
	}
...
func (p *Plan) createShardGroupMetadata(client server.MetaClient) error {
	ts := p.StartTime.Truncate(p.ShardDuration).UTC()

	var err error
	groups := make([]*meta.ShardGroupInfo, p.ShardCount)
	for i := 0; i < p.ShardCount; i++ {
		groups[i], err = client.CreateShardGroup(p.Database, p.info.DefaultRetentionPolicy, ts)
		if err != nil {
			return err
		}
